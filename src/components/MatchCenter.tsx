...
const MatchCenter = () => {
  ...
  const MatchCard = ({ match }: { match: VirtualMatch }) => (
    <div 
      className={cn(
        "group relative py-3 sm:py-4 px-3 sm:px-4 cursor-pointer transition-all duration-300",
    ...
    >
      ...
      {match.status === 'live' && (
        <div className="absolute left-0 top-1/2 -translate-y-1/2 w-1 h-8 sm:h-10 bg-emerald-500 rounded-r-full">
      ...
      <div className="flex items-center justify-between mb-2 sm:mb-3">
        <div className="flex items-center gap-2 min-w-0 flex-1">
          <div className="flex items-center gap-1.5">
            <Trophy className="w-3 h-3 sm:w-3.5 sm:h-3.5 text-amber-500/80" />
            <span className="text-[10px] sm:text-xs font-semibold text-amber-500/90 uppercase tracking-wider truncate max-w-[140px] sm:max-w-none">
              {match.league}
            </span>
          </div>
        ...
        <div className="flex items-center gap-2 flex-shrink-0">
          ...
          <div className="flex items-center gap-1 text-muted-foreground/60">
            ...
            <span className="text-[10px] sm:text-xs">{match.heat}K</span>
          </div>
        </div>
      </div>
      ...
      <div className="flex items-center gap-2 sm:gap-3">
        ...
        <div className="flex-1 flex items-center justify-end gap-1.5 sm:gap-2 min-w-0">
          <div className="flex items-center gap-1">
            {match.homeRedCards !== undefined && match.homeRedCards > 0 && (
              <span className="inline-flex items-center justify-center w-4 h-4 sm:w-5 sm:h-5 text-[9px] sm:text-xs font-bold bg-red-500 text-white rounded">
                {match.homeRedCards}
              </span>
            )}
            {match.homeYellowCards !== undefined && match.homeYellowCards > 0 && (
              <span className="inline-flex items-center justify-center w-4 h-4 sm:w-5 sm:h-5 text-[9px] sm:text-xs font-bold bg-amber-400 text-black rounded">
                {match.homeYellowCards}
              </span>
            )}
          </div>
          {match.homeRank && (
            <span className="text-[8px] sm:text-[10px] text-muted-foreground/50 font-medium">[{match.homeRank}]</span>
          )}
          <span className="text-xs sm:text-sm font-medium text-foreground/90 truncate group-hover:text-foreground transition-colors">
            {match.homeTeam}
          </span>
        </div>

        {/* Score */}
        <div className="flex-shrink-0 min-w-[56px] sm:min-w-[70px] flex flex-col items-center">
          {match.status === 'upcoming' || (match.homeScore === undefined && match.awayScore === undefined) ? (
            <div className="flex flex-col items-center">
              {activeTab === 'upcoming' && (
                <span className="text-[8px] sm:text-[10px] text-muted-foreground/50 mb-0.5 uppercase tracking-wider">Soon</span>
              )}
              <span className="text-xs sm:text-sm text-muted-foreground/60 font-medium tracking-widest">vs</span>
            </div>
          ) : (
            <div className="flex flex-col items-center">
              {activeTab === 'finished' && (
                <span className="text-[8px] sm:text-[10px] text-muted-foreground/50 mb-0.5 uppercase tracking-wider">FT</span>
              )}
              <div className={cn(
              ...
              {((match.halfTimeScore && match.halfTimeScore !== '0-0') || match.corners) && (
                <span className="text-[8px] sm:text-[10px] text-muted-foreground/50 mt-1 font-medium">
        ...
        <div className="flex-1 flex items-center gap-1.5 sm:gap-2 min-w-0">
          ...
          {match.awayRank && (
            <span className="text-[8px] sm:text-[10px] text-muted-foreground/50 font-medium">[{match.awayRank}]</span>
          )}
          <div className="flex items-center gap-1">
        ...
        {match.hasVideo && (
          <button 
            ...
            className="flex-shrink-0 w-7 h-7 sm:w-8 sm:h-8 rounded-lg bg-emerald-500/10 flex items-center justify-center hover:bg-emerald-500/20 transition-colors border border-emerald-500/20"
          >
      ...
      {/* Match ID */}
      <div className="flex items-center justify-start mt-2 sm:mt-2.5">
        <span className="text-[8px] sm:text-[10px] text-muted-foreground/40 font-mono tracking-wider">
          ID {match.id.slice(-4)}
        </span>
      </div>
    </div>
  ...
  return (
    <Card className="h-[550px] sm:h-[700px] flex flex-col border-border/40 bg-gradient-to-b from-card to-card/95 backdrop-blur-xl shadow-xl overflow-hidden">
      {/* Tab navigation - Modern European style */}
      <div className="flex items-center bg-muted/30 px-1 sm:px-2 overflow-x-auto border-b border-border/30">
        {tabs.map((tab) => {
          ...
          return (
            <button
              ...
              className={cn(
                "relative flex items-center gap-1.5 px-3 sm:px-4 py-3 sm:py-3.5 text-[11px] sm:text-xs font-medium whitespace-nowrap transition-all duration-300",
                isActive 
                  ? "text-emerald-400" 
            ...
            >
              ...
              {tab.id === 'favorites' && favoritesCount > 0 && (
                <span className="inline-flex items-center justify-center min-w-[16px] h-[16px] px-1 text-[9px] font-bold bg-emerald-500 text-white rounded-full">
                  {favoritesCount}
                </span>
              )}
              {isActive && (
                <div className="absolute bottom-0 left-1/2 -translate-x-1/2 w-8 sm:w-10 h-0.5 bg-gradient-to-r from-emerald-400 to-teal-400 rounded-full" />
              )}
            </button>
          );
        })}
