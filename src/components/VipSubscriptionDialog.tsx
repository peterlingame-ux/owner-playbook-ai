...
const VipSubscriptionDialog = ({ open, onOpenChange, isVipActive, onVipPurchased }: VipSubscriptionDialogProps) => {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="w-[calc(100vw-32px)] max-w-sm p-0 overflow-hidden border-0 bg-gradient-to-b from-background via-background to-muted/30 mx-auto">
        {/* Privileges List - Compact and responsive */}
        <div className="px-3 pt-4 pb-2 space-y-1.5">
          {privileges.map((privilege, index) => (
            ...
            >
              <div 
                className={`flex-shrink-0 w-6 h-6 rounded-md bg-gradient-to-br ${privilege.color} flex items-center justify-center shadow-md`}
              >
                <privilege.icon className="w-3 h-3 text-white" />
              </div>
              <div className="flex-1 min-w-0 overflow-hidden">
                <div className="flex items-center gap-1">
                  <h3 className="font-medium text-foreground text-[11px] truncate">{privilege.title}</h3>
                  {isVipActive && (
                    <Check className="w-2.5 h-2.5 text-green-500 flex-shrink-0" />
                  )}
                </div>
                <p className="text-[9px] text-muted-foreground truncate">{privilege.description}</p>
              </div>
            </motion.div>
          ))}
        </div>
        ...
        <div className="px-3 pb-3 pt-1.5">
          {!isVipActive && (
            <>
              {/* Price Display */}
              <div className="flex items-center justify-center gap-1.5 mb-2 p-1.5 rounded-lg bg-muted/50 border border-border/30">
                <span className="text-muted-foreground text-[10px]">{t('vip_price')}:</span>
                <div className="flex items-center gap-0.5">
                  <img src={hunterCoinIcon} alt="Hunter Coin" className="w-3.5 h-3.5" />
                  <span className="text-base font-bold text-primary">{VIP_COST}</span>
                </div>
                <span className="text-muted-foreground text-[9px]">/ {VIP_DURATION_DAYS}{t('days')}</span>
              </div>
              ...
              <Button
                ...
                className="w-full h-8 text-xs font-bold relative overflow-hidden"
                style={{
          ...
          {isVipActive && (
            <div className="text-center p-2 rounded-lg bg-gradient-to-r from-cyan-500/10 via-blue-500/10 to-cyan-500/10 border border-cyan-500/20">
              <div className="flex items-center justify-center gap-1">
                <Check className="w-3.5 h-3.5 text-green-500" />
                <span className="font-semibold text-foreground text-xs">{t('vip_status_active')}</span>
              </div>
              <p className="text-[9px] text-muted-foreground mt-0.5">{t('enjoy_all_privileges')}</p>
            </div>
          )}
        </div>
      </DialogContent>
